...
public class ConfirmViewController: UIViewController, UIScrollViewDelegate {
    ...
    internal func confirmPhoto() {
        ...
        if allowsCropping {
            ...
            let normalizedWidth = ((kPortraitWidth - 120) / imageView.frame.width)
            let normalizedHeight = ((kPortraitWidth - 180)*2 / (imageView.frame.height))
            
            let rect = normalizedRect(CGRect(x: normalizedX, y: normalizedY, width: normalizedWidth, height: normalizedHeight), orientation: .up)
